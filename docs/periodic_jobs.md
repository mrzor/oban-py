...
> **Tip:** Cron syntax can be difficult to write and read. We recommend using a tool like
...
specific time. This is useful for initialization tasks that should only run once per cluster
startup.
...
```python
oban = Oban(pool=pool, scheduler={"timezone": "America/New_York"})
```
...
```python
@worker(cron={"expr": "0 9 * * MON-FRI", "timezone": "America/New_York"})
...
To run periodic jobs, you need to ensure your workers are loaded before starting Oban.
...
* **Resolution Limit**: Cron scheduling has a one-minute resolution at minimum. For more frequent
  executions, consider an alternative approach.
